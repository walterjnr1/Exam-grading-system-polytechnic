...
$rowaccess = mysqli_fetch_array($result);
 ...
 if(isset($_POST["btncheck"])){
        ...
        $semester = $_POST['cmdsemester'];
        
        $sql = "SELECT * FROM `tblresult` WHERE `semester`=? AND `school_session`=? AND `admissionID`=?";
                    $query = $dbh->prepare($sql);
                    $query->execute(array($semester,$session,$admissionID));
                    $row = $query->rowCount();
                    $fetch = $query->fetch();
                    if($row > 0) {
			...
			$sql = "select * from school_session ";
			 ...
			 ?>  
                       ...
                       <div class="form-group"><strong>
                                    ...
                                    <select name="cmdsemester" class="form-control" id="semester" >
