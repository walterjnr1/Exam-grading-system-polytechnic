...
{
if($_POST['txtadmissionID'] != "" || $_POST['txtpassword'] != ""){

$admissionID = $_POST['txtadmissionID'];
$password = $_POST['txtpassword'];
$status = 1;

$sql = "SELECT * FROM `tblstudent` WHERE `admissionID`=? AND `password`=? AND `status`=?";
			$query = $dbh->prepare($sql);
			$query->execute(array($admissionID,$password,$status));
			$row = $query->rowCount();
			$fetch = $query->fetch();
			if($row > 0) {
			
		$_SESSION['login_admissionID'] = $fetch['admissionID'];
...
$message = ' <div class="alert alert-danger">Invalid Admission No/Password</div> ';
           ...
           <div class="form-holder">
                <div class="form-content">
                    <div class="form-items"><h4 ><?php echo $message; ?></h4>
                      <p align="center" ><a href="index.php"><span class="logos"><img class="logo-size" src="<?php echo $logo; ?>" alt="" height="90" width="90"></span></a></p>
                      <h3><?php echo $website_name ;?> - Student Login Form</h3>
                        <p>&nbsp;</p>
                        <p> </p>
                        <div class="page-links">
                            <a href="index.php" class="active">Student Login</a><a href="register.php">Student Registration</a><a href="admin.php">Admin </a>
                        </div>
                       		             <form  action="" method="POST"  class="registration-form">
                            <input class="form-control" type="text" name="txtadmissionID" placeholder="Admission No" required>
